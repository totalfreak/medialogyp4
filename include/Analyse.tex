...
\\
As mentioned, many people will learn more with kinesthetic learning where movement and hands on using more senses will improve their learning outcome and learning experience\toddo{det er der da ikke blevet sagt - ikke skriv "as meantioned når der slet ikke er blevet nævnt movement}]. The amount of movement used in kinesthetic learning can depend on the assignment as well as the learning goal and can also vary depending on the amount of movement that is incorporated in the exercise\\
...
\subsection{Motivation theory} \todo{ lidt kedelig titel. måske den skulle vinkles til læring for også at styrke ideen om at alle disse emner stadig handler om hvordan man lære bedst} 
...
\subsection*{Intrinsic motivation} \todo{det er altså forvirrende at underoverskrifterne er lige så store som deres overoverskrifter (og hvorfor har de ikke længere tal?)}
...
\todo{som ovenstående kommentar siger, så er det ret vigtigt at vi for nævnt at vores tilgang til målgruppen (sankt Annæ) ikke er en  variet repræsentation af netop målgruppen. Det er her vigtigt at nævne at disse redakaber vi nævner, ikke nødvendigvis bliver brugt i den almindelige folkeskole (eg nodelære).}
...
\subsubsection{Lego - Music notation} \todo{husk at høre interviewet med Hanna igen! jeg husker det altså som om at de ikke bruger fysiske klodser, men at hun kun tegner dem på tavlen !}
Lego is a variance of physical plastic blocks that are used for building a variety of designs. They come in different lengths and shapes and the idea is to build whatever the individual desires. Electronic components like motors, LED lights etc. have been made for adding more control and innovation for the individual creator. The usefulness of Lego is the capability of using it for anything creative such as understanding music theory. 
...
\subsubsection{Clio Online} \todo{jeg forstår ikke hvad det gør ? er det video materiale? er det et forum, quizzer? hvad er det ? }
...
\subsubsection{Little bits} \todo{hør interview, jeg er ikke sikkerpå at hendes elever bruger det? tror kun hun nævnte det som redskab i andre samenhænge (specielt fordi det ikke rigtig har nogen sammenhæng med musik ?)}
Little bits are different bits of electronic components that can be connected to each other to create different interactive designs. Each module is an electronic circuit and each of the modules comes in four different colors. Blue is for power, pink is for input, green is for output and orange is the connector. This is for learning children about circuits, innovation and how each component functions in a very low level environment. 
...
\todo{ jeg synes det mangler lidt om hvad det går ud på - hvordan udføre man denne metode? }
...
\todo{samme som ovenover. DERTIL tænkte jeg om det egentlig ikke er her gameification kunne komme ind i billedet ? }
...
\end{figure}
	...
	\todo{for alle redskaber i SOTAen, synes jeg der mangler reflektioner for hvordan og hvilke elementer der kunne være interesante at arbejde ind i vores tool. }
...
\section{Design Requirements}
	\subsection*{Functional Requirements}
		\begin{itemize}
			...
			\item[-] It must output sound. \todo{det har vi egentlig talt om, men skal den også nødvendigvis kunne det ? hvad i vores analyse siger at det er et must?}
