...
\begin{figure}[H]
	...
	\caption{Retention of Learning from Kokcharov's Skillpyramid}
...
Hanna is very clear, when she says that the children have an easier time learning, when working with physical objects. Another tool Hanna uses are Lego blocks. She draws them on a blak board and uses it to explain steps in music. \\
...
In order to establish a set target group, the study plan was investigated, and a music’s teacher was interviewed, to highlight issues with current musical education. It was found that there was a gap when it came to physical tools that elementary school students could use in collaborate with each other. A workshop was held with students at Sank Annæ Skole, where several design and functionality idea were presented. The feedback provided, gave an understanding of what kind of product the students themselves wanted, on top of what the teacher had already provided as feedback. Therefore, it was chosen that the solution to be created, would be a physical tool, that the students could use in collaboration with each other in an elementary school setting.\\

Further investigation lead to a deeper understanding in the concepts of group work, group roles, classroom settings, and the three different collaborative learning methods; Cooperative learning, Constructive learning and Peer learning.\\
